...
- **`packages.txt`** / **`requirements.txt`** / **`allowed-domains.txt`** / **`node-version.txt`** / **`codex-version.txt`** — claude-container 同梱のデフォルト apt/pip パッケージ・許可ドメイン一覧（空のフォールバック既定値）と、Node.js（`24.18.0`）・Codex CLI（対応版 `0.155.1`）の固定版 default。プロジェクト側で上書きする場合は `.c3c/` を使う（「利用側プロジェクトの設定」参照。`node-version.txt`・`codex-version.txt` は空ファイルが opt-out）。`allowed-ports.txt` にはこの種の同梱デフォルトは無く、プロジェクト側に無ければ `claude-container` がビルドコンテキスト内に空ファイルをその場で生成する（`init-firewall.sh` 自身が既定値 `443,22` を適用する、という意味。警告は出さない）。
