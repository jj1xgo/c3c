...
`init-firewall.sh` の `resolve_allowed_ports()`・`refresh_domains()` を編集した場合も `./test-build.sh --launcher-only` を実行する。ポート検証と DNS 応答の回帰テスト（`tests/test-refresh-domains.sh`）を含む。DNS 応答のテストだけなら `bash tests/test-refresh-domains.sh` で実行できる。実際の iptables と接続の確認は別途コンテナで行う。
