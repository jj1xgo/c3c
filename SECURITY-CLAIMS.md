...
`~/.claude/settings.json` を 0 バイトで作るため）。0 バイトや空白だけの `.mcp.json` は対象にしない（従来の `.mcp.json` ゲートと同じ）。判定不能・停止のエラー文も、repo 側が決めた名前を含みうるので制御文字を除いてから出す。環境変数による opt-out は無く、`CLAUDE_PROJECT_APPROVAL_FILE`・
