...
- **`claude-container`**
  ...
  - 旧入口の廃止予告（`guard_legacy_entry()`）は `C3C_INTERFACE` だけで分岐する案内で、起動可否は判定しないため dispatch 直前のガード呼び出し列には含めない。通常起動・`--clean` は dispatch 冒頭、`--check` は `check_one_project()` 冒頭の `readonly CHECK_MODE` 後と、ドライバの初期対象0件時に呼ぶ。旧 parser・CLI 選択記憶・終了コードの契約は変えない。
