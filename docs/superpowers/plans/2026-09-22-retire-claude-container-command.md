...
状態: 段階Aの予告・診断・文書・回帰テストをローカル実装済み。実機検証・レビュー・公開は未完了。段階Bは起動方法の聞き取りまで、段階Cは未着手。日付による廃止期限は設けず、下記の移行・検証条件を満たした次のメジャー版で削除する。
...
- `c3c --check` でも移行確認事項を表示し、旧入口による直接実行が検出された場合と、外部スクリプト・alias の利用状況が未確認であることを区別する。警告が出ないことを全利用側の移行完了とは扱わない。
...
主な変更: `claude-container`、`tests/test_c3c_launch.py`、`test-build.sh` の診断テスト、README.md。実装前に [開発不変条件](../../development-invariants.md) の launcher・check・選択記憶の節を読む。
