...
| Step | 実測 |
| --- | --- |
| 1 `--check` | `[OK]   git 設定: GITCONFIG_FILE 未設定（空の <c3c>/empty.gitconfig を ~/.gitconfig へ :ro で bind）`。WARN は fixture の `CODEX_DIR` のパーミッションだけ（700 にして解消）。ビルド後の再実行でも `→ 結果: PASS` で、ドリフトの WARNING は出ない。既存イメージの再ビルド不要: 変更したファイルは `ASSET_HASH_TARGETS` に含まれない（`git diff --name-only` と照合）。main（b69d168）の c3c でビルド済みの `localhost/c3c-3bb23052_claude-auth-workspace` に、main の c3c と本ブランチの c3c でそれぞれ `--check` を当てると、出力は完全に一致した（どちらにも出る境界アセットのドリフト WARNING は本変更より前からのもの） |
